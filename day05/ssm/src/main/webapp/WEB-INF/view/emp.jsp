...
<body class="gray-bg">
				...
				<div>
					<div class="input-group">
			     	 <span class="input-group-addon">时间 </span>
				       	<input required="required" placeholder="开始时间" id="txt_search_start" name="start" class="laydate-icon form-control layer-date"/>
	                    <input required="required" placeholder="结束时间"id="txt_search_end" name="end" class="laydate-icon form-control layer-date"/>
					</div>
			 	 </div>
			 </div>
					...
					<form id="form_user" method="post" action="reserveEmp.htm">
						...
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							<tr>
								...
								<td>生日：</td>
								<td>
							       	<input required="required" placeholder="开始时间" id="txt_search_start1" name="birthday" class="laydate-icon form-control layer-date"/>
								</td>
							</tr>
							<tr>
								<td>性别：</td>
								...
								<td>爱好：</td>
								<td>
									<input type="checkbox" name="hobby" id="hobby1" value="足球">足球
									<input type="checkbox" name="hobby" id="hobby2" value="篮球">篮球
							...
							<tr>
								...
								<td>头像：</td>
								<td>
									<input type="text" name="pic" id="pic" class="form-control" aria-required="true" required>
								</td>
							</tr>
							<tr>
								<td>部门：</td>
								<td colspan="4">
									<select class="form-control" name="dDepid" id = "dDepid">
										<option value="0">---请选择---</option>
										<c:forEach items="${dList }" var="r">
									 	<option value="${r.dDepid }">${r.dDname }</option>
										</c:forEach>
    ...
    laydate({
        elem: '#txt_search_start1', //目标元素。由于laydate.js封装了一个轻量级的选择器引擎，因此elem还允许你传入class、tag但必须按照这种方式 '#id .class'
        event: 'focus', //响应事件。如果没有传入event，则按照默认的click
        format: 'YYYY-MM-DD'// 日期格式
    });
	...
	function initEditUser(getSelection){
		...
		$('#txt_search_start1').val(new Date(getSelection.birthday).Format("yyyy-MM-dd HH:mm:ss"));
		...
		$('input[value='+getSelection.gender+'][type=radio]').attr("checked",true);
