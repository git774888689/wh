...
<body class="gray-bg">
					...
					<form id="form_user" method="post" action="reserveStudent.htm">
						...
						<table style="border-collapse:separate; border-spacing:0px 10px;">
							<tr>
								...
								<td>学生生日：</td>
								<td>
									<input placeholder="开始时间" readonly="readonly" id="txt_search_start" name="birthday" class="laydate-icon form-control layer-date"/>
								</td>
							</tr>
							<tr>
								<td>学生年龄：</td>
								...
								<td>学生性别：</td>
								<td>
									<input type="radio" name="sex" id="sex" value="男" aria-required="true">男
									<input type="radio" name="sex" id="sex" value="女" aria-required="true">女
								</td>
							</tr>
							<tr>
								<td>学校：</td>
								<td colspan="4">
									<select class="form-control" name="schoolid" id = "schoolid" aria-required="true" required>
	...
	function initEditUser(getSelection){
		...
		$('#txt_search_start').val(new Date(getSelection.birthday).Format("yyyy-MM-dd"));
		$('#name').val(getSelection.name);
		$('#age').val(getSelection.age);
		$('input[value='+getSelection.sex+']').attr("checked",true);
		$('#schoolid').val(getSelection.schoolid);
